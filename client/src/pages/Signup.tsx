...
export default function Signup() {
  ...
  const inputStyle = (field: string, hasError: boolean) => ({
    border: hasError ? "1.5px solid #ef4444" : focusedField === field ? "1.5px solid #4F46E5" : "1.5px solid #e2e8f0",
    boxShadow: focusedField === field
      ? "0 0 0 3px rgba(79,70,229,0.12), 0 2px 8px rgba(79,70,229,0.08)"
      : hasError ? "0 0 0 3px rgba(239,68,68,0.1)" : "none",
    background: hasError ? "#fef2f2" : "#fff",
  });

  const validate = () => {
    const e: { name?: string; email?: string; password?: string } = {};
    if (!name || name.trim().length < 2) e.name = "Nome deve ter pelo menos 2 caracteres";
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#F8FAFC] overflow-hidden">
      ...
      >
        <div className="absolute inset-0" style={{
          background: "linear-gradient(135deg, #0B1E3C 0%, #1e1b4b 35%, #0f2d1f 70%, #0B1E3C 100%)",
          backgroundSize: "400% 400%",
          animation: "gradientShift 8s ease infinite",
        }} />
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute rounded-full"
            style={{ width: 400, height: 400, top: "10%", left: "10%", background: "radial-gradient(circle, rgba(79,70,229,0.25) 0%, transparent 70%)", filter: "blur(40px)", animation: "pulseBlob1 5s ease-in-out infinite" }} />
          <div className="absolute rounded-full"
            style={{ width: 300, height: 300, bottom: "15%", right: "5%", background: "radial-gradient(circle, rgba(34,197,94,0.2) 0%, transparent 70%)", filter: "blur(50px)", animation: "pulseBlob2 7s ease-in-out infinite 1s" }} />
        </div>
        ...
        <div className="relative z-10 flex flex-col items-center text-center px-10 py-12">
          ...
          >
            Crie sua conta e acesse a plataforma completa de saúde auditiva ocupacional
          </p>
          <div
            className="flex flex-wrap gap-2 mt-8 justify-center"
            style={{ animation: "fadeIn 0.7s ease-out 0.9s both" }}
          >
            {["Gratuito para começar", "Dados isolados", "Laudos PDF"].map((tag) => (
      ...
      >
        <div className="w-full max-w-[420px]">
          ...
          <div className="mb-8" style={{ animation: "fadeInUp 0.6s ease-out 0.3s both" }}>
            ...
            <h2 className="text-3xl font-bold text-[#0B1E3C] mb-2">Criar conta</h2>
            <p className="text-[#64748b] text-sm">Preencha os dados para começar a usar o sistema</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5" style={{ animation: "fadeInUp 0.6s ease-out 0.4s both" }}>
            {/* Name */}
            <div>
              <label className="block text-sm font-semibold text-[#1e293b] mb-1.5">Nome completo</label>
              <input
                type="text"
                value={name}
              ...
              />
              {errors.name && (
                <p className="text-xs text-red-500 mt-1.5 transition-all duration-200">{errors.name}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.email && (
                <p className="text-xs text-red-500 mt-1.5 transition-all duration-200">{errors.email}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.password && (
                <p className="text-xs text-red-500 mt-1.5 transition-all duration-200">{errors.password}</p>
              )}
            </div>
